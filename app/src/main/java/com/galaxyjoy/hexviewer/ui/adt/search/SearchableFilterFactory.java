...
public class SearchableFilterFactory {
     ...
     */
    private SearchResult performsSearch(final LineEntry line,
                                        String query,
                                        Locale loc,
                                        int hexLength) {
        ...
        return new SearchResult(length,
                indexes.isEmpty() ? null : indexes,
                hexPart,
                withSpaces,
                mSearchFromHewView.isSearchNotFromHexView());
    }
    ...
    public void multilineSearch(final LineEntry lineEntry,
        ...
        if (mSearchFromHewView.isSearchNotFromHexView())
            lineWidth = UIHelper.getMaxByLine(mContext,
                    mUserConfigLandscape,
                    mUserConfigPortrait) + 1;
        else {
     ...
     * @param tempList    The output list.
     */
    private void evaluateResult(final int shiftOffset,
                                int i,
                                SearchResult sr,
                                final String query,
                                final Set<Integer> tempList) {
    ...
    private void prepareEvaluateResultHexPart(final int shiftOffset,
                                              final SearchResult sr,
                                              final AtomicInteger nbPerLines,
    ...
    private void prepareEvaluateResultTextPart(final int shiftOffset,
                                               final SearchResult sr,
                                               final AtomicInteger nbPerLines,
                                               final AtomicInteger index) {
    ...
    private void finalizeEvaluateResult(final int i,
                                        final int index,
                                        final int nbPerLines,
                                        final String query,
                                        final Set<Integer> tempList) {
